...
function install_pgadmin() {
    ...
    sudo sh -c 'echo "deb [signed-by=/usr/share/keyrings/packages-pgadmin-org.gpg] https://ftp.postgresql.org/pub/pgadmin/pgadmin4/apt/$(lsb_release -cs) pgadmin4 noble-pgdg-testing" > /etc/apt/sources.list.d/pgadmin4.list && apt update && apt list --upgradable'
